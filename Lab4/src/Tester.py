...
class Test_model(VAE_Model):
    ...
    def eval(self):
        ...
        cat_seq = torch.cat(pred_seq_list)
        
        # Security Catch: Prevent NaNs from exploding gradients turning into nulls in Kaggle parser
        if torch.isnan(cat_seq).any():
            print("WARNING: NaN detected in generation. Filling with 0 to prevent Kaggle Null Errors.")
            cat_seq = torch.nan_to_num(cat_seq, nan=0.0)
            
        # Security Catch: Clamp pixels to [0,1] to prevent invalid negative/large values 
        cat_seq = torch.clamp(cat_seq, 0.0, 1.0)

        pred_to_int = (np.rint(cat_seq.numpy()*255)).astype(int)
